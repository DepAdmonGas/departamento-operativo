...
function Estacion($idEstacion, $con){
 $sql = "SELECT razonsocial FROM tb_estaciones WHERE id = '".$idEstacion."' ";
$result = mysqli_query($con, $sql);
while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
...
$sql_firma = "SELECT * FROM op_pedido_pinturas_complementos_firma WHERE id_pedido = '".$idReporte."' AND tipo_firma = '".$CatFirma."' ";
...
$Detalle = '<div class="border-bottom text-center p-2" style="margin-top: 10px;"><small>El pedido de pinturas y complementos se firmó por un medio electrónico.</br> <b>Fecha: '.FormatoFecha($explode[0]).', '.date("g:i a",strtotime($explode[1])).'</b></small></div>';
...
$contenido .= '<div class="text-center" style="font-size: 1.8em;">Pedido de pinturas</div>';

$sql = "SELECT * FROM op_pedido_pinturas_complementos WHERE id = '".$GET_idReporte."' ";
$result = mysqli_query($con, $sql);
...
$contenido .= '<div class="text-center" style="font-size: 1.2em;margin-top:10px;margin-bottom:10px;">'.Estacion($idEstacion, $con).'</div>';
...
$contenido .= '<td><b>Unidad</b></td>';
$contenido .= '<td><b>Nombre Producto</b></td>';
...
$contenido .= '<td class="align-middle">'.$row_lista['unidad'].'</td>';
$contenido .= '<td class="align-middle">'.$row_lista['producto'].'</td>';
...
$contenido .= '<tr><td colspan="3" class="text-right">Total piezas:</td><td class="text-center"><b>'.$ToPiezas.'</b></td></tr>';
$contenido .= '</tbody>';
$contenido .= '</table>';
...
$dompdf->stream("Pedido de pinturas ".Estacion($idEstacion, $con).".pdf");
