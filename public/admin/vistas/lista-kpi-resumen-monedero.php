...
function consultaPuntajeMensual($GET_idEstacion,$GET_year,$GET_mes,$con){
  ...
  //---------- GRAFICAS TODAS LAS ESTACIONES ----------//
    ...
    function mostrarGraficoFacturasMonederoES($GET_year, $con) {
    $data = obtenerDatosFacturasMonederoES($GET_year, $con);
    ...
    echo '<div class="border p-0">';
    echo '<div class="table-responsive">';
    echo '<div id="chart_div_estaciones' . $GET_idEstacion . '' . $GET_year . '"></div>';
    echo '</div>';
    echo '</div>';
    echo '</div>'; 
...
<?php echo mostrarGraficoFacturasMonederoES($GET_year,$con)?>
