...
$sql_firma = "SELECT * FROM op_pedido_limpieza_firma WHERE id_pedido = '".$idReporte."' AND tipo_firma = '".$CatFirma."' ";
...
$Detalle = '<div class="border-bottom text-center p-2" style="margin-top: 10px;"><small>El pedido de limpieza se firmó por un medio electrónico.</br> <b>Fecha: '.FormatoFecha($explode[0]).', '.date("g:i a",strtotime($explode[1])).'</b></small></div>';
...
$contenido .= '<div class="text-center" style="font-size: 1.8em;">Pedido de limpieza</div>';

$sql = "SELECT * FROM op_pedido_limpieza WHERE id = '".$GET_idReporte."' ";
$result = mysqli_query($con, $sql);
...
$sql_lista = "SELECT * FROM op_pedido_limpieza_detalle WHERE id_pedido = '".$GET_idReporte."' ";
...
$contenido .= '<td class="align-middle">'.$Producto['unidad'].'</td>';
$contenido .= '<td class="align-middle">'.$Producto['producto'].'</td>';
...
$dompdf->stream("Pedido de limpieza ".Estacion($idEstacion, $con).".pdf");
