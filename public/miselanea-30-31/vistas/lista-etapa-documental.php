...
?>
  ...
  <li aria-current="page" class="breadcrumb-item active text-uppercase">Etapa documental (<?=$estacion;?>), <?=$idYear;?></li>
  </ol>
  </div>

  <div class="row">
  <div class="col-12">
  <h3 class="text-secondary" style="padding-left: 0; margin-bottom: 0; margin-top: 0;">Etapa documental (<?=$estacion;?>), <?=$idYear;?></h3>
  </div>
  ...
  <table id="tabla_documentos_<?=$idEstacion ?>_<?=$idYear?>" class="custom-table" style="font-size: 12.5px;" width="100%"> 
  ...
  <?php
  $sql_lista = "SELECT * FROM op_miselanea_documentos WHERE categoria = 1 ORDER BY id_lista ASC";
  $result_lista = mysqli_query($con, $sql_lista);
  $numero_lista = mysqli_num_rows($result_lista);
  ...
  echo '<tr>';
  echo '<th class="align-middle text-center">'.$row_lista['id_lista'].'.</th>';
