...
<div class="col-2">
<button type="button" class="btn btn-outline-primary btn-sm float-end" onclick="ModalReporte(<?=$idEstacion;?>)">Reporte</button>
</div>
...
<?php
	...
	while($row_asistencia = mysqli_fetch_array($result_asistencia, MYSQLI_ASSOC)){
    	...
    	$incidencia = '<small><span class="badge rounded-pill bg-info fw-light p-1 float-end"> </span></small>';
    	}else{
    	$incidencia = '';
    	}
	...
	<td class="align-middle fs-6 text-center">'.$incidencia.$iconIncidencia.'</td>
	</tr>';
