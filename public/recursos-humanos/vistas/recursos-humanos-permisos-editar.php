...
$sql_lista = "SELECT * FROM op_rh_permisos WHERE id = '".$GET_idPermiso."' ORDER BY id DESC";
...
$Responsable = Nombre($idpersonal,$con);
...
<html lang="es">
 ...
 function Crear(idPermiso){
  ...
  $('#Observacion').css('border','2px solid #A52525'); 
  }
  }else{
...
<div class="row">
    ...
    <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
     <label class="text-secondary mt-2">* Colaborador</label> 
      <?php 
      $sql_listaestacion = "SELECT id, nombre FROM tb_usuarios WHERE id_gas = '".$idestacion."' AND estatus = 0 ORDER BY nombre ASC";
      $result_listaestacion = mysqli_query($con, $sql_listaestacion);
      echo '<select class="form-select titulos" id="Colaborador">';
     
      echo '<option value="'.$idpersonal.'">'.$nameUSUR.'</option>';
      while($row_listaestacion = mysqli_fetch_array($result_listaestacion, MYSQLI_ASSOC)){
...
<div class="row">
  ...
  <option value="<?=$idComodin?>"><?=$nameUSU;?></option>
  </select>
  </div>
...
<button type="button" class="btn btn-primary" onclick="Crear(<?=$GET_idPermiso;?>)">Editar permiso</button>
</div>
