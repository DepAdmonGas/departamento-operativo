...
function tablaPersonal($idEstacion,$info,$Buscar,$ocultartb,$con){
	
    if($Buscar != ""){
    $Like = " AND (op_rh_personal.nombre_completo like '%".$Buscar."%' OR op_rh_puestos.puesto like '%".$Buscar."%')";  
    }else{
    $Like = "";
    }
	...
	WHERE op_rh_personal.id_estacion = '".$idEstacion."' $Like AND op_rh_personal.estado = '".$info."' ORDER BY op_rh_personal.id ASC ";
...
if($idTipo == 1){
    echo tablaPersonal($idEstacion,1,$Buscar,$ocultartb,$con);

}else if($idTipo == 0){
    echo tablaPersonal($idEstacion,0,$Buscar,$ocultartb,$con);
