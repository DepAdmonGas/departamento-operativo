...
$(document).ready(function($){
  ...
  function VentasSubTotales(idReporte){
    $('#TrSubTotales').load('../../../app/vistas/personal-general/1-corporativo/corte-diario/concentrado-ventas-subtotales.php?idReporte=' + idReporte);
...
function VentasTotales(idReporte){
    $('#TrTotales').load('../../../app/vistas/personal-general/1-corporativo/corte-diario/concentrado-ventas-totales.php?idReporte=' + idReporte);
...
<tbody>
    ...
    ?>
    	...
    	<td class="p-0"> 
		<select class="form-select" id="producto-<?=$idventas;?>" style="border: 0px;width: 100%;padding: 3px;font-size: 1.2em;" onchange="EditProducto(this,<?=$idReporte;?>,<?=$idventas;?>)" <?=$estado;?>>
				<?php if ($producto == "") { ?>
    	...
    	<input id="litros-<?=$idventas;?>" type="number" min="0" step="any" style="border: 0px;width: 100%;padding: 3px;height: 100%; text-align: right;" onkeyup="EditLitros(this,<?=$idReporte;?>,<?=$idventas;?>)" value="<?=$litros;?>" <?=$estado;?>>
    	</td>

    	<td class="p-0 align-middle">
    		<input id="jarras-<?=$idventas;?>" type="number" min="0" step="any" style="border: 0px;width: 100%;padding: 3px;height: 100%;text-align: right;" onkeyup="EditJarras(this,<?=$idReporte;?>,<?=$idventas;?>)" value="<?=$jarras;?>" <?=$estado;?>>
    	</td>

    	<td class="bg-light align-middle text-end" id="totallitros-<?=$idventas;?>"><strong><?=number_format($totalLitros, 2);?></strong></td>
    	<td class="p-0 align-middle">
    		<input id="preciolitro-<?=$idventas;?>" type="number" min="0" step="any" style="border: 0px;width: 100%;padding: 3px;height: 100%;text-align: right;" onkeyup="EditPrecioLitro(this,<?=$idReporte;?>,<?=$idventas;?>)" value="<?=$preciolitro;?>" <?=$estado;?>>
    	</td>
    	<td class="bg-light align-middle text-end" id="importetotal-<?=$idventas;?>"><strong><?=number_format($importeTotal, 2);?></strong></td>
	...
	<tr>
		...
		<td class="align-middle <?=$cssaceite;?>">
    		...
    		<input id="preciootros-<?=$idOtros;?>" type="number" min="0" step="any" style="border: 0px;width: 100%;padding: 3px;height: 100% !important;text-align: right;" onkeyup="EditPrecioOtros(this,<?=$idReporte;?>,<?=$idOtros;?>)" value="<?=$importe;?>" <?=$estado;?>>
        <?php } ?>
    	</td>
	</tr>
	<?php } ?>
