	...
	<tbody>
		...
		if ($numero_credito > 0) {
			while ($row_credito = mysqli_fetch_array($result_credito, MYSQLI_ASSOC)) {
				...
				$CTipo = "";
				if (mb_convert_encoding($row_credito['tipo'], 'UTF-8', 'ISO-8859-1')== "Crédito") {
					$CTipo = "text-primary";
				} else if (mb_convert_encoding($row_credito['tipo'], 'UTF-8', 'ISO-8859-1') == "Débito") {
					$CTipo = "text-success";
				}
...
<td class="align-middle font-weight-light ' . $CTipo . ' ">' . mb_convert_encoding($row_credito['tipo'], 'UTF-8', 'ISO-8859-1') . '</td>
